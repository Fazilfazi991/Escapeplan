...
const controlledBase=strengths=>normalizeAssessment(raw('I want out of my job eventually','Full-time job','₹1L–₹3L','2–3 hours/day','Combine both',strengths,'Balanced',{runway:['5–7 months']}));
const modelById=id=>models.find(model=>model.business_model_id===id);
assert(scoreV2(controlledBase(['Technology / systems']),modelById('ai-utility-website')).factors.skills>scoreV2(controlledBase(['Nothing obvious yet']),modelById('ai-utility-website')).factors.skills,'Technology strength must improve AI Utility skill affinity');
assert(scoreV2(controlledBase(['Finding products / deals']),modelById('private-label-product')).factors.skills>scoreV2(controlledBase(['Nothing obvious yet']),modelById('private-label-product')).factors.skills,'Product-sourcing strength must improve product-led skill affinity');
const ui=[];
for(const [id,input] of personas){
 ...
 assert.equal(one.engine.scoring_model_version,'2.0.1');assert.equal(one.score_semantics.is_success_probability,false);assert(one.top_business.reasons.every(x=>x.factor&&Number.isFinite(x.score)&&x.label));assert(one.alternative_teasers.every(x=>one.ranked_businesses.some(r=>r.business_model_id===x.business_model_id)));assert(one.avoid_models.every(x=>x.reasons.length));
