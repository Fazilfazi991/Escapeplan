...
const rawKey='escapeplan-assessment-v1',assessmentKey='escapeplan_assessment_v2',resultKey='escapeplan_result_v2',legacyStateKey='escapeplan_assessment_v2';
...
async function calculate(){
  ...
  result.generated_at=new Date().toISOString();const record={record_version:'escapeplan-result-state-v2',saved_at:result.generated_at,versions:ENGINE_VERSIONS,raw_answers:raw.answerText,canonical_assessment:assessment,result};
  const assessmentRecord={record_version:'escapeplan-assessment-state-v2',saved_at:result.generated_at,versions:ENGINE_VERSIONS,raw_answers:raw.answerText,canonical_assessment:assessment};
  localStorage.setItem(assessmentKey,JSON.stringify(assessmentRecord));sessionStorage.setItem(assessmentKey,JSON.stringify(assessmentRecord));localStorage.setItem(resultKey,JSON.stringify(record));sessionStorage.setItem(resultKey,JSON.stringify(record));return record;
}
async function loadRecord(){for(const storage of [localStorage,sessionStorage]){for(const key of [resultKey,legacyStateKey]){try{const value=JSON.parse(storage.getItem(key)||'null');if(value?.versions?.scoring_model_version===ENGINE_VERSIONS.scoring_model_version&&value?.versions?.quiz_schema_version===ENGINE_VERSIONS.quiz_schema_version&&value?.result?.top_business)return value}catch(_){}}}return calculate();}
function failure(error){document.body.innerHTML='<main class="cv-error"><h1>We couldn’t finish your match just now.</h1><p>Your answers are still saved.</p><button type="button" id="retry-analysis">Retry Analysis</button><button type="button" id="restart-assessment">Start Again</button></main>';document.getElementById('retry-analysis').addEventListener('click',()=>{localStorage.removeItem(resultKey);sessionStorage.removeItem(resultKey);location.href='/analysis-v2/code'});document.getElementById('restart-assessment').addEventListener('click',()=>{[rawKey,assessmentKey,resultKey].forEach(key=>{localStorage.removeItem(key);sessionStorage.removeItem(key)});location.href='/assessment/code'});console.error('EscapePlan scoring failed:',error.message);}
...
function renderDNA(record){
  const {result,canonical_assessment:a}=record;set('profile-title',result.profile_archetype.label);set('profile-copy',profileCopy(a,result));
  const qualitativeFill={High:85,Moderate:60,Low:35};document.getElementById('dna-bars').innerHTML=result.business_dna.slice(0,4).map(x=>`<div style="--fill:${Math.round(x.value??qualitativeFill[x.band])}%"><span>${x.label.toUpperCase()}<b>${x.band}</b></span><i><em></em></i></div>`).join('');
...
function renderPaywall(record){
  ...
  const teaserQuestions=result.alternative_teasers.map(teaser=>teaser.label);while(teaserQuestions.length<3)teaserQuestions.push('Which strong alternative fits your answers differently?');const questions=[...teaserQuestions.slice(0,3),'What should you NOT start right now?'];document.querySelectorAll('.question-locks strong').forEach((node,index)=>node.textContent=questions[index]);
